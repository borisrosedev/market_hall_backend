...
def create_one_product(
    *, session: Session, uploaded_file: UploadFile, product_create: ProductCreateRequest
) -> ProductDTO:
    ...
    save_file(
        ...
        uploaded_file_specific_folder="products",
    )
    db_obj = Product.model_validate(product_create, update={"photo_name": unique_filename})
    session.add(db_obj)
    session.commit()
