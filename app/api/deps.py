...
def get_current_user_from_cookie(request: Request, session: SessionDep) -> User:
    ...
    user = (
        session.get(User, ident=user_id)
        or session.exec(select(User).where(User.id == user_id)).first()
    )
    if not user:
...
def get_current_user(session: SessionDep, token: TokenDep) -> User:
    ...
    user = (
        session.get(User, user_id) or session.exec(select(User).where(User.id == user_id)).first()
    )
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
    if getattr(user, "is_banned", False):
