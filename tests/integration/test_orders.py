...
@pytest.mark.integration
def test_create_order_with_user_id(client,make_user_id):
    user_id, email, password = make_user_id() 
    ...
    # order created 
    payload = { "user_id":user_id,"amounts_cents":80000,"currency":"ALL","status":"created"}
    r_order = client.post("/api/v1/orders/", json=payload)
    assert r_order.status_code in (200, 201), f"{r_order.status_code} body={r_order.data!r}"
